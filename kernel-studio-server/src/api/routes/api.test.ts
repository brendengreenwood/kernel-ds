...
describe('Observation routes', () => {
  beforeEach(async () => {
    await addObservations([
      { transcriptId: 't1', personaId: 'merchant', perspective: 'standard', title: 'S1', body: 'Standard for t1' },
      { transcriptId: 't1', personaId: 'merchant', perspective: 'reality', title: 'R1', body: 'Reality for t1' },
      { transcriptId: 't1', personaId: 'merchant', perspective: 'experience', title: 'E1', body: 'Experience for t1' },
      { transcriptId: 't2', personaId: 'csr', perspective: 'standard', title: 'S2', body: 'Standard for t2' },
    ]);
  });

  it('GET /api/observations returns all observations', async () => {
    const res = await req('/api/observations');
    const data = await res.json();
    expect(data).toHaveLength(4);
  });

  it('GET /api/observations?transcript=t1 filters by transcript', async () => {
    const res = await req('/api/observations?transcript=t1');
    const data = await res.json();
    expect(data).toHaveLength(3);
    expect(data.map((o: { perspective: string }) => o.perspective)).toEqual(['standard', 'reality', 'experience']);
  });

  it('GET /api/observations?persona=merchant filters by persona', async () => {
    const res = await req('/api/observations?persona=merchant');
    const data = await res.json();
    expect(data).toHaveLength(3);
  });

  it('GET /api/observations?perspective=standard filters by perspective', async () => {
    const res = await req('/api/observations?perspective=standard');
    const data = await res.json();
    expect(data).toHaveLength(2);
    expect(data.every((o: { perspective: string }) => o.perspective === 'standard')).toBe(true);
  });

  it('GET /api/observations/transcript/:id returns takeaways for session', async () => {
    const res = await req('/api/observations/transcript/t1');
    const data = await res.json();
    expect(data).toHaveLength(3);
  });

  it('GET /api/observations/recent returns latest observations', async () => {
    const res = await req('/api/observations/recent');
    const data = await res.json();
    expect(data).toHaveLength(4);
  });

  it('GET /api/observations/recent?limit=2 respects limit', async () => {
    const res = await req('/api/observations/recent?limit=2');
    const data = await res.json();
    expect(data).toHaveLength(2);
  });
});
