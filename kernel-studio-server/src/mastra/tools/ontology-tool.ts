...
import { z } from 'zod';
import { Persona, type PersonaType, getPersonaLabels } from './transcript-tool';
import {
...
export const compareRolePerspectivesTool = createTool({
  ...
  execute: async ({ entityId, roleA, roleB }) => {
    ...
    return {
      entity: entity.name,
      roleA: { role: roleA, label: (PERSONA_LABELS[roleA] || roleA), perspective: perspA },
      roleB: { role: roleB, label: (PERSONA_LABELS[roleB] || roleB), perspective: perspB },
      divergenceAreas,
      existingTensions,
    };
  },
...
export const compareToolEcosystemTool = createTool({
  ...
  execute: async ({ personas }) => {
    const PERSONA_LABELS = await getPersonaLabels();
    const allTools = await getAllTools();
    const targetPersonas = personas || await listPersonaIds();
    ...
    targetPersonas.forEach(p => {
      ...
      if (specific.length > 0) {
        personaSpecificTools[(PERSONA_LABELS[p] || p)] = specific;
      }
    });
