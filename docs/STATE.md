...
- **UI package contract in progress** (decision 0042, 2026-07-30): `packages/ui` defines the future `@kernel/ui` distribution as ESM, declarations, CSS, and a catalog-backed `api.json`, with explicit root/marks/icon/utils/style exports. React and React DOM are peer-only; package tests and CI reject wildcard exports, private portal-source leakage, bundled or dependency-owned React, undeclared runtime imports, missing public artifacts, and payload files outside the allowlist. This phase still compiles the canonical portal source through a transitional build alias; the active extraction phase moves those implementations into the package and removes that alias.
