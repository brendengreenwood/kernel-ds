...
- **Guidance citations are gated** (decision 0069, 2026-09-03): `ds:doctor` now runs `agents-citations`, which reads every `AGENTS.md` and `CLAUDE.md` outside the generated markers and fails when a cited path or `npm run` script does not resolve (`scripts/ds/lib/agents-citations.mjs`; red fixture `scripts/ds/__fixtures__/agents-stale-citation/`). Closes decision 0038's deferral. Its first run found a dozen lines left behind by the ui-package extraction (token sheet, icon shim, marks check, ui primitives, portal `hooks/` and `pages/gallery/`, studio `mastra/public/`), all fixed in the same commit. Authoring rule: root files cite repo-relative paths, package files cite package-relative ones, and a prose word pair with a slash is written as words.
