...
- **Figma and repository agents now share an explicit DSDS reconciliation brief** (2026-09-22): `docs/figma/DSDS-AGENT-HANDOFF.md` defines catalog IDs as the join keys across canonical code, the Kernel DS Figma library, and DSDS-informed product designs. It gives each agent separate responsibilities, requires a read-only identity/drift inventory before mutation, defines a one-entity vertical-slice proof, and standardizes handoffs without making Figma or prototypes silently canonical.
