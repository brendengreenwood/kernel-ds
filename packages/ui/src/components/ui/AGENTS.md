...
Motion is token-driven — animate with duration-[var(--duration-base)] ease-[var(--ease-out)], never ad-hoc ms/curves. The prefers-reduced-motion guard in src/styles.css covers new motion for free
...
After changing a primitive's variants or slots, re-run the parity gate from the repo root: node kernel-portal/scripts/check-component-docs.mjs
