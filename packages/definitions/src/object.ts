 ...
 */
export function deriveCoord(id: string): { x: number; y: number } {
  return {
    x: 5 + (djb2(`x:${id}`) % 91),
    y: 5 + (djb2(`y:${id}`) % 91),
  }
...
/** Return whether an unknown value satisfies the object definition contract. */
export function validateObjectDefinition(value: unknown): boolean {
  return objectDefinitionSchema.safeParse(value).success
}
...
export function parseObjectModel(json: string): {
  ...
  const parsed = objectDefinitionSchema.parse(JSON.parse(json))
