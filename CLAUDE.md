...
- Two brand roles (decision 0032): **green is traversal, blue is action**. `--brand-*` (Cargill green) colors nav/chrome (`--sidebar-*`, `--secondary`, `--accent`); `--action-*` (cobalt blue, full 50→950) drives everything interactive — `--primary`, `--ring`, links, focus (all rings incl. `--sidebar-ring`). Don't point action roles back at green or use `--action-*` for chrome.
