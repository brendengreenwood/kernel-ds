...
- Two brand roles, kept apart (Cargill — decision 0016): **action = orange** (`--primary`/`--ring` → `--orange-*`; buttons, links, focus), **navigation = green** (`--sidebar-*`/`--secondary`/`--accent` → `--brand-*`; rail, active item, logo). Green = go somewhere, orange = do something — don't flip `--primary` back to green or paint the sidebar orange.
