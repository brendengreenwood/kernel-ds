...
- **Green world, blue actions** (decision 0035, superseding 0033/0034's green-button tiers): green is everything you move through and the state you set — chrome, nav, links, tabs, selection controls, focus (`--primary`/`--ring` stay brand green). **Buttons are actions and never wear green**: the Button ladder is 1 primary `default` (action-blue fill; one blue lead per region) · 2 secondary `outline` (white; any number) · 3 tertiary `ghost` (quietest). Never two blues side by side; never repoint `--primary` at `--action-*` (it drives the green non-button state); never use action blue for chrome, nav, links, or selection. Green text-links use the traversal ink: `text-brand-700 dark:text-brand-300`. The brand-tinted `variant="secondary"` is outside the ladder — toned chips/toggled states only, never action clusters.
