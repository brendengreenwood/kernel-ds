...
- **Mostly green, surgical blue** (decision 0033, amending 0032): the system is green — `--primary`, `--ring`, chrome, nav, links, selection, focus all ride `--brand-*` roles. The `--action-*` cobalt scale is reserved for **at most one element per view**: the decisive/committing action, via `Button variant="action"` (Post bid, Price selected, Settle, Confirm). Never repoint `--primary` at `--action-*`, never use action blue for navigation or routine controls. Green text-links (breadcrumbs, record links) use the traversal ink: `text-brand-700 dark:text-brand-300`.
- **Action hierarchy** (decision 0034): four tiers of button emphasis — 1 decisive `variant="action"` (blue, one per view) · 2 primary `default` (green, ~one per region) · 3 secondary `outline` (white; any number) · 4 tertiary `ghost` (quietest). Adjacent buttons differ by a tier; never two solids side by side; beside a blue commit the green tier is skipped (ghost · outline · action). The brand-tinted `variant="secondary"` is outside the ladder — toned chips/toggled states only, never action clusters.
