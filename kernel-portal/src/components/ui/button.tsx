...
import { BeamWrap, type BorderBeamProp } from "@/components/ui/border-beam"

const buttonVariants = cva(
  "group/button inline-flex shrink-0 items-center justify-center rounded-lg border border-transparent bg-clip-padding text-sm font-medium whitespace-nowrap transition-all outline-none select-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 active:not-aria-[haspopup]:translate-y-px disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      /* Buttons are actions (decision 0035) — green never appears on a
         button. The ladder, descending emphasis:
         1 `default` — action blue: the lead action of a region
         2 `outline` — secondary: white surface, any number of supporting actions
         3 `ghost`   — tertiary: quietest — cancel, dismiss, inline utilities
         `secondary` (brand-tinted) sits outside the ladder: toned chips and
         toggled states, not action clusters. Green is traversal ink — nav,
         links, tabs, chrome — never a button fill. */
      variant: {
        default:
...
function Button({
  ...
  return (
    <BeamWrap
      beam={borderBeam}
      defaults={{ size: "sm", className: "inline-flex align-middle" }}
    >
      <ButtonPrimitive
        data-slot="button"
