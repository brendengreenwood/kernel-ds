App chrome + doc/gallery/foundations pages + the objects/ workspace. Composes @kernel/ui primitives
...
foundations.tsx + *-foundation.tsx — token showcases (color/type/spacing/shadow/motion/icons/a11y/layout). MUST stay in sync when the @kernel/ui token sheet (packages/ui/src/styles.css) changes
